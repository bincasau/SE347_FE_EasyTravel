...
export default function HotelsImportExcelButton({
  ...
  const onPick = async (e) => {
    ...
    // ✅ popup loading
    const close = popup.loading("Đang import khách sạn...");
    ...
    try {
        ...
        async ({ row, idx }) => {
          ...
          if (!hotelData.name || !hotelData.address) {
            return {
              idx,
              status: "fail",
              message: `Row ${idx + 2}: thiếu Name/Address`,
            };
          }
      ...
      // ✅ đóng loading TRƯỚC khi show kết quả (tránh chớp)
      close?.();

      const summary = aborted
        ? `Đã hủy import.\nĐã xử lý: ${ok + fail}/${tasks.length}\nOK: ${ok}, FAIL: ${fail}`
        : `Import xong: ${ok} OK, ${fail} FAIL (Tổng ${tasks.length})`;
      ...
      if (errors.length) {
        ...
        popup.success(summary, "Kết quả import");
      }

      if (typeof onImported === "function") onImported({ ok, fail, errors });
    } catch (err) {
      close?.();
      popup.error(err?.message || "Import failed");
    } finally {
      setImporting(false);
      abortRef.current.aborted = false;
    }
  ...
  const label = importing
    ? progress.phase === "reading"
      ? "Đang đọc file..."
      : `Đang import... ${percent}% (${progress.done}/${progress.total})`
    : "Import Excel";
