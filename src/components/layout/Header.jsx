...
import { getUserFromToken, isLoggedIn, setAuthFlag, setToken } from "@/utils/auth";
import { X } from "lucide-react";
import NotificationPanel, { NotificationPanelMobile } from "./NotificationPanel";
...
export default function Header({ onOpenLogin, onOpenSignup }) {
  ...
  const fetchUser = async () => {
    ...
    try {
      ...
      setUser({
        ...
        role: jwtUser?.role || apiUser?.role || null,
      });
    } catch {
      setUser(null);
    }
  ...
  /**
   * ✅ Load notifications only when user changes
   */
  useEffect(() => {
    if (user) {
      // Reset loading state
  ...
  // ✅ Main useEffect: handle JWT token from URL and listen for changes
  useEffect(() => {
